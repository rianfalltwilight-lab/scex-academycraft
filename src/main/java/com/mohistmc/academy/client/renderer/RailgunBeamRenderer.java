...
public class RailgunBeamRenderer extends EntityRenderer<RailgunBeamEntity> {
    ...
    private static final ResourceLocation BEAM_TEXTURE =
            ResourceLocation.fromNamespaceAndPath(AcademyCraft.MODID, "textures/effects/railgun/tile.png");
    ...
    private void renderBillboardBeam(Matrix4f matrix, VertexConsumer consumer,
        ...
        for (int i = 0; i < segments; i++) {
            ...
            consumer.addVertex(matrix, s0 + bx, s1 + by, s2 + bz)
                    ...
                    .setUv(t0, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight)
                    .setNormal(0, 1, 0);
            consumer.addVertex(matrix, s0 - bx, s1 - by, s2 - bz)
                    .setColor(r, g, b, alpha)
                    .setUv(t0, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight)
                    .setNormal(0, 1, 0);
            consumer.addVertex(matrix, e0 - bx, e1 - by, e2 - bz)
                    .setColor(r, g, b, alpha)
                    .setUv(t1, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight)
                    .setNormal(0, 1, 0);
            consumer.addVertex(matrix, e0 + bx, e1 + by, e2 + bz)
                    .setColor(r, g, b, alpha)
                    .setUv(t1, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLight)
                    .setNormal(0, 1, 0);
        }
    }
