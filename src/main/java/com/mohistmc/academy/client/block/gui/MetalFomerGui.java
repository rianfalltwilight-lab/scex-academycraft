...
public class MetalFomerGui extends AcademyBaseUI<MetalFomerMenu> {
    ...
    protected void renderLabels(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        if (isHoveringButton(this.leftPos + ICON_X, this.topPos + ICON_Y, ICON_SIZE, ICON_SIZE, mouseX, mouseY)) {
            ...
            // renderLabels already has the GUI pose translated; only the hover
            // test above uses absolute mouse coordinates.
            graphics.fill(x - 6, y, x + tw + 6, y + this.font.lineHeight + 4, 0x80000000);
            graphics.drawString(this.font, txt, x, y + 2, 0xCCFFFFFF, false);
        }
    }

    @Override
    public void renderBackground(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        renderStandardMachinePanel(graphics, UI_METAL_FORMER);
