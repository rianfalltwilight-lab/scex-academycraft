...
public class PhaseLiquidRender implements BlockEntityRenderer<PhaseLiquidBlockEntity> {
    ...
    public void render(PhaseLiquidBlockEntity blockEntity, float partialTick, PoseStack poseStack,
        ...
        float alpha = PhaseLiquidVisualMath.distanceAlpha(distanceSquared, FADE_DISTANCE_SCALE);
        ...
        drawLayer(poseStack, bufferSource, 0, -0.30f * liquidHeight, 0.30f, 0.20f, alpha);
        drawLayer(poseStack, bufferSource, 1, 0.35f * liquidHeight, 0.30f, 0.05f, alpha);
        if (liquidHeight > TOP_LAYER_THRESHOLD) {
            drawLayer(poseStack, bufferSource, 2, 0.70f * liquidHeight, 0.10f, 0.25f, alpha);
        }
    }

    private void drawLayer(PoseStack poseStack, MultiBufferSource bufferSource, int layer,
                           float height, float speedU, float speedV, float alpha) {
        ...
        addVertex(vertexConsumer, pose, 0.0F, height, 0.0F, uOffset, vOffset, alpha);
        addVertex(vertexConsumer, pose, 1.0F, height, 0.0F, uOffset + UV_SPAN, vOffset, alpha);
        addVertex(vertexConsumer, pose, 1.0F, height, 1.0F, uOffset + UV_SPAN, vOffset + UV_SPAN, alpha);
        addVertex(vertexConsumer, pose, 0.0F, height, 1.0F, uOffset, vOffset + UV_SPAN, alpha);
    }
    ...
    private void addVertex(VertexConsumer vertexConsumer, PoseStack.Pose pose,
        ...
        vertexConsumer.addVertex(pose, x, y, z)
                // 1.0.7's RenderImagPhaseLiquid feeds the camera-distance
                // fade directly to glColor4d.  Keeping a fixed alpha here made
                // distant layers stay unnaturally bright even though the fade
                // had already been calculated above.
