...
public final class InfoArea {
    ...
    public void draw(GuiGraphics gg, int leftPos, int topPos) {
        int px = resolvePanelX(leftPos);
        if (px == Integer.MIN_VALUE) {
            // At extreme GUI scales there is no non-overlapping side for the
            // optional information card. Never cover inventory hit boxes;
            // the machine page remains fully usable until the scale is lowered.
            return;
        }
        int py = topPos + Y;
    ...
    /** Shared by interactive node properties so draw and hit-test use exactly
     * the same side when the card falls back from right to left. */
    public static int resolvePanelX(int leftPos) {
        int preferred = leftPos + X;
        int scaledWidth = Minecraft.getInstance().getWindow().getGuiScaledWidth();
        if (preferred + W <= scaledWidth) return preferred;
        if (leftPos - 7 - W >= 0) return leftPos - 7 - W;
