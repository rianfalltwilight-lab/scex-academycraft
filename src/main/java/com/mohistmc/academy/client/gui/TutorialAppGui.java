...
public class TutorialAppGui extends AcademyScreen {
    ...
    public void tick() {
        ...
        if (++refreshTicks >= 20) {
            refreshTicks = 0;
            refreshTutorialGroups(true);
        }
    }

    private void refreshTutorialGroups(boolean preserveSelection) {
        var player = Minecraft.getInstance().player;
        if (player == null) return;
        boolean wasLearned = currentTut != null && learned.contains(currentTut);
        List<ACTutorial>[] groups = TutorialRegistry.groupByLearned(player);
        learned.clear();
        unlearned.clear();
        learned.addAll(groups[0]);
        unlearned.addAll(groups[1]);
        boolean nowLearned = currentTut != null && learned.contains(currentTut);
        if (preserveSelection && currentTut != null && wasLearned != nowLearned) {
            contentScroll = 0;
            smoothScroll = 0;
            previewIndex = 0;
            rebuildViews();
    ...
    private void ensureSelectedListItemVisible() {
        int idx = learned.indexOf(currentTut);
        if (idx < 0) {
            int unlearnedIndex = unlearned.indexOf(currentTut);
            if (unlearnedIndex >= 0) idx = learned.size() + unlearnedIndex;
        }
        if (idx < 0) return;
        int top = 7 + idx * (int) LIST_ITEM_H;
        int bottom = top + (int) LIST_ITEM_H;
        int visible = (int) LIST_H - 9;
        if (top < listScroll) listScroll = top;
        else if (bottom > listScroll + visible) listScroll = bottom - visible;
    ...
    public void render(GuiGraphics g, int mx, int my, float pt) {
        ...
        fscale = (float) Math.min(width / REF_WIDTH, height / (double) FRAME_H);
    ...
    private void drawLeftList(GuiGraphics g, int mx, int my) {
        com.mojang.blaze3d.systems.RenderSystem.enableBlend();
        com.mojang.blaze3d.systems.RenderSystem.defaultBlendFunc();
        drawPanel(g, 0, 0, (int) LIST_W, (int) LIST_H);

        int sx = (int) fx;
        int sy = (int) (fy + 2 * fscale);
        g.enableScissor(sx, sy, (int) (fx + LIST_W * fscale), (int) (fy + (LIST_H - 2) * fscale));
        g.pose().pushPose();
        g.pose().translate(0, -listScroll, 0);
        float y = 7;
        y = drawListGroup(g, mx, my, learned, true, y, 0);
        y = drawListGroup(g, mx, my, unlearned, false, y, learned.size());
        g.pose().popPose();
        g.disableScissor();
        maxListScroll = Math.max(0, (int) Math.ceil(y + 7 - LIST_H));
    }
    ...
    private float drawListGroup(GuiGraphics g, int mx, int my, List<ACTutorial> list, boolean isLearned, float startY, int startIdx) {
        ...
        for (ACTutorial t : list) {
            ...
            boolean hov = isHoveredInFrame(mx, my, x, y - listScroll, LIST_ITEM_W, LIST_ITEM_H);
    ...
    private int renderContentLine(GuiGraphics g, TutorialMdParser.TutorialLine line, float y) {
        ...
        return switch (line.type()) {
            ...
            case TABLE_ROW -> {
                String value = String.join("  |  ", line.cells());
                int dy = 0;
                for (var s : wrapScaled(resolveMisaka(TutorialMdParser.processInline(value)), w - 4, 8)) {
                    drawText(g, s, x + 2, y + dy + 1, 0xFFD5DADF, 8);
                    dy += 9;
                }
                g.fill(x, (int) y + dy, x + w, (int) y + dy + 1, 0x5534495E);
                yield dy + 3;
            }
            case TABLE_SEP -> 3;
            case IMAGE -> {
                var resource = Minecraft.getInstance().getResourceManager().getResource(line.image());
                if (resource.isPresent()) {
                    var dimensions = RenderUtils.textureSize(line.image());
                    int imageH = dimensions.width() > 0 && dimensions.height() > 0
                            ? Math.max(1, Math.round(w * dimensions.height() / (float) dimensions.width()))
                            : 72;
                    // Markdown illustrations have very different aspect ratios
                    // (for example ability_ui is portrait while overload is a
                    // wide strip).  Stretching every image to 154x72 visibly
                    // corrupted both; fit to content width and preserve ratio.
                    GuiRenderHelper.blitTranslucent(g, line.image(), x, (int) y, w, imageH);
                    yield imageH + 4;
                }
                drawText(g, "§7[" + (line.alt().isBlank() ? "missing image" : line.alt()) + "]", x, y, 0xFF8A9299, 9);
                yield 12;
            }
            case CODE_BLOCK -> {
    ...
    private void drawShowWindow(GuiGraphics g, int mx, int my) {
        ...
        if (!currentViews.isEmpty()) {
            ...
            com.mojang.blaze3d.systems.RenderSystem.enableBlend();
            com.mojang.blaze3d.systems.RenderSystem.defaultBlendFunc();
        } else if (isCurrentLearned() && !visiblePreviews().isEmpty()) {
            String key = "ac.tutorial.no_recipe";
            String message = Component.translatable(key).getString();
            if (message.equals(key)) message = "No compatible recipe available";
            int tw = font.width(message);
            drawText(g, message, ax + Math.max(2, (areaW - tw * 9f / 16f) / 2f), ay + areaH / 2f,
                    0xFF8A9299, 9);
        }

        // 标签页(showWindow 内 x=12, Return setPos(12, 120.75))
        if (currentTut != null) {
            List<ViewGroup> pv = visiblePreviews();
            float step = TAG_H - 1;
            float x = sx;
            com.mojang.blaze3d.systems.RenderSystem.enableBlend();
            com.mojang.blaze3d.systems.RenderSystem.defaultBlendFunc();
    ...
    private void drawRightWindow(GuiGraphics g) {
        ...
        drawPanel(g, (int) wx, (int) wy, (int) RIGHT_WIN_W, (int) RIGHT_WIN_H);
    ...
    public boolean mouseScrolled(double mx, double my, double sx, double sy) {
        ...
        float top = fy + 2 * fscale, bottom = fy + (CENTER_H - 2) * fscale;
        float listRight = fx + LIST_W * fscale;
        if (mx >= fx && mx <= listRight && my >= top && my <= fy + LIST_H * fscale) {
            listScroll = (int) Math.clamp(listScroll - sy * 20, 0, maxListScroll);
            return true;
        }
        if (currentTut != null && mx >= contentX && mx <= contentRight && my >= top && my <= bottom) {
    ...
    private void drawPanel(GuiGraphics g, int x, int y, int w, int h) {
        g.fill(x, y, x + w, y + h, 0xE0182331);
        g.fill(x, y, x + w, y + 1, 0xAA60758C);
        g.fill(x, y + h - 1, x + w, y + h, 0xAA101820);
        g.fill(x, y, x + 1, y + h, 0xAA60758C);
        g.fill(x + w - 1, y, x + w, y + h, 0xAA101820);
    }
    ...
    private int findHoveredListIndex(double mx, double my) {
        if (mx < fx || mx > fx + LIST_W * fscale || my < fy + 2 * fscale || my > fy + (LIST_H - 2) * fscale) return -1;
        double adjustedY = my + listScroll * fscale;
        int idx = 0;
        for (ACTutorial t : learned) {
            if (isHoveredInFrame(mx, adjustedY, 6.6f, 7 + idx * LIST_ITEM_H, LIST_ITEM_W, LIST_ITEM_H)) return idx;
            idx++;
        }
        int base = idx;
    ...
    private boolean isCurrentLearned() {
        return currentTut != null && learned.contains(currentTut);
    }

    /** Locked tutorials retain descriptive item/block previews, never recipe contents. */
    private List<ViewGroup> visiblePreviews() {
        if (currentTut == null) return List.of();
        if (isCurrentLearned()) return currentTut.getPreview();
        return currentTut.getPreview().stream()
                .filter(group -> group.recipeTarget().isEmpty())
                .toList();
    }
