...
public class TutorialMdParser {
    ...
    private static final Map<String, TutorialData> CACHE = new ConcurrentHashMap<>();
    ...
    public static TutorialData parse(String fileId) {
        ...
        return CACHE.computeIfAbsent(lang + "\u0000" + fileId, ignored -> load(fileId, lang));
    }

    /** Called when client resources/language are reloaded. */
    public static void clearCache() {
        CACHE.clear();
    }

    private static TutorialData load(String fileId, String lang) {
    ...
    private static TutorialData missing(String id) {
        return new TutorialData(id, id, "Tutorial resource unavailable",
                List.of(new TutorialLine(Type.H2, "", null, "Tutorial unavailable"),
                        new TutorialLine(Type.TEXT, "", null,
                                "The tutorial resource could not be loaded. Try reloading resources or reinstalling the mod.")));
    }
    ...
    static String processInline(String text) {
        ...
        // Resolve structured key tags before italic parsing: ids such as
        // open_data_terminal contain underscores and must stay atomic.
        s = processKeyTags(s);
    ...
    private static String processKeyTags(String text) {
        ...
        while (true) {
            ...
            if (idStart != -1) {
                ...
                if (idEnd != -1) keyName = translatedKey(keyContent.substring(idStart + 4, idEnd));
            }
    ...
    private static String translatedKey(String id) {
        var mapping = switch (id) {
            case "ability_activation", "toggle_ability" -> KeyInputHandler.TOGGLE_ABILITY;
            case "edit_preset", "skill_slot" -> KeyInputHandler.OPEN_SKILL_SLOT;
            case "switch_preset" -> KeyInputHandler.SWITCH_PRESET;
            case "open_data_terminal", "open_terminal" -> KeyInputHandler.OPEN_TERMINAL;
            default -> null;
        };
        return mapping == null ? id : mapping.getTranslatedKeyMessage().getString();
    }
