...
public class WindGenBaseSubBlock extends BaseEntityBlock {
    ...
    /** Jade/WAILA and pick-block should present this proxy as its logical machine. */
    @Override public String getDescriptionId() { return "block.academy.windgen_base"; }
    ...
    public void neighborChanged(BlockState state, Level level, BlockPos pos, Block block, BlockPos neighbor, boolean p_60514_) {
        if (!level.isClientSide && !(level.getBlockState(pos.below()).getBlock() instanceof WindGenBase)) {
    ...
    public boolean onDestroyedByPlayer(BlockState state, Level level, BlockPos pos, Player player,
        ...
        WindGenBase main = getMainBlock(level, pos);
        if (!level.isClientSide && main != null) {
            if (!player.getAbilities().instabuild) {
                Block.popResource(level, pos.below(), new ItemStack(main.asItem()));
            }
